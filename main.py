...
# 儲存為 SRT 格式
def save_as_srt(transcriptions, output_path):
    with open(output_path, 'w', encoding='utf-8') as f:
        for i, segment in enumerate(transcriptions, 1):
            start = format_timestamp(segment["start"])
            end = format_timestamp(segment["end"])
            text = f"Speaker {segment['speaker']}: {segment['text']}"
            f.write(f"{i}\n{start} --> {end}\n{text}\n\n")
    print(f"SRT file saved to {output_path}")

# 儲存為 VTT 格式
def save_as_vtt(transcriptions, output_path):
    with open(output_path, 'w', encoding='utf-8') as f:
        f.write("WEBVTT\n\n")
        for i, segment in enumerate(transcriptions, 1):
...
# 解析命令行參數
parser = argparse.ArgumentParser(description="語音轉文字及說話者辨識工具")
parser.add_argument('--format', type=str, choices=['csv', 'srt', 'vtt'], default='csv',
                    help="選擇輸出格式：csv、srt 或 vtt（預設: csv）")
...
for file in files:
    ...
    if output_format == 'csv':
        ...
        # 設定輸出字幕檔案路徑
        subtitle_extension = 'srt' if output_format == 'srt' else 'vtt'
        output_subtitle_file = os.path.join(output_folder, f"{os.path.splitext(file)[0]}_speaker_transcription.{subtitle_extension}")
        
        # 根據選擇的格式儲存字幕
        if output_format == 'srt':
            save_as_srt(matched_result, output_subtitle_file)
        elif output_format == 'vtt':
            save_as_vtt(matched_result, output_subtitle_file)
